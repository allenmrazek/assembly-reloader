 ...
 * "working" MainView, which changes rapidly according to last-minute
 * design considerations. In both cases, Mediators help you to insulate
 * the MainView from the app around it, and vice-versa. This yields
 ...
 * is the MainView class, the Value is one or more Mediators. Whenever the
 * MainView shows up, a corresponding Mediator joins it to buffer
 * MainView from app. The MainView doesn't know about the Mediator, nor
 ...
 * to know quite a lot about the MainView and the app. It can be injected,
 ...
 * enough to provide mediation. Leave logic, data storage, and MainView
 * behaviour to other classes.
 * 
 * In the context of Unity3D, the MainView is a MonoBehaviour attached
 ...
 * practice to attach only the MainView to your GameObject in Unity3D.
 * 
 ...
 * nor any other class involving the world beyond the MainView's immediate
 ...
 * what in MainView is the trickiest part of this system. I recommend the
...
namespace AssemblyReloader.StrangeIoC.extensions.mediation.api
{
	...
	{
		/// An event that just happened, and the MainView it happened to.
