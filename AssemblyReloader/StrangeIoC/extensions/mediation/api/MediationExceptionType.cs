...
namespace AssemblyReloader.StrangeIoC.extensions.mediation.api
{
	...
	{
		/// Exception raised when a MainView can't locate a Context.
		/// Views contact the Context by "bubbling" their existence up 
		/// the display chain (recursively using transform.parent).
		/// If a MainView reaches the top of that chain without locating
		/// a Context, it will raise this Exception to warn you.
		/// 
		/// Note: to avoid infinite looping, there is a bubbling limit of 100
		/// layers. If your MainView needs to be more than 100 transforms deep, 
		/// that might signal a design problem.
		NO_CONTEXT,

		/// Exception raised when a MainView is mapped to itself.
		/// If a MainView is accidentally mapped to itself, the result will be an
		...
		/// MainView bound to abstraction that MainView doesn't actually extend/implement
		VIEW_NOT_ASSIGNABLE,
	}
}
